...
export default class FriendButton extends React.Component {
    ...
    componentDidMount() {
        if (!this.props.friendshipStatus || this.props.friendshipStatus == 0) {
            ...
            this.props.friendshipStatus == 3 ||
            this.props.friendshipStatus == 4 ||
            this.props.friendshipStatus == 5
        ) {
    ...
    handleClick() {
        if (!this.props.friendshipStatus || this.props.friendshipStatus == 0) {
            axios
                .post(`/send-request/${this.props.id}`)
                .then(res => {})
                .catch(err => console.log(err));
        } else if (this.props.friendshipStatus == 1) {
            if (this.props.otherId == this.props.recipientId) {
                axios
                    .post(`/cancel-request/${this.props.id}`)
                    .then(res => {
                ...
                axios
                    .post(`/accept-request/${this.props.id}`)
                    .then(res => {
            ...
            axios
                .post(`/delete-friend/${this.props.id}`)
                .then(res => {})
                .catch(err => console.log(err));
